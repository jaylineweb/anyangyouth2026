...
$(function(){
	...
	$(function() {
		...
		$window.on("scroll.common", function(event) {
			...
			// [추가] 콘솔에 현재 스크롤 위치와 헤더 높이를 출력합니다.
			var headerHeight = $('#header').outerHeight() || 0;
			console.log("현재 스크롤 위치: " + scrollTop + "px / 기준 헤더 높이: " + headerHeight + "px");
			...
			if ($quick.length) {
				if (scrollTop > headerHeight) {
					$quick.attr('data-scroll', 'fixed');
				} else {
					// 오타 수정: removaAttr -> removeAttr
					$quick.removeAttr('data-scroll');
				}
			}
		}).triggerHandler('scroll.common');
		/* 검색 */
		var $searchContainer = $('.search'),
			$searchBtn = $('.search_open .search_btn'),
			$searchInput = $('#search_query'),
			$closeBtn = $('.search_close_btn');

		if($searchContainer.length && $searchBtn.length) {
			$searchBtn.off('click.searchToggleCommon').on('click.searchToggleCommon', function() {
				$searchContainer.toggleClass('active');

				if($searchContainer.hasClass('active') && $searchInput.length) {
					setTimeout(function() {
						$searchInput.focus();
					}, 100);
				}
			...
			$closeBtn.off('click.searchToggleCommon').on('click.searchToggleCommon', function() {
				$searchContainer.removeClass('active');
				$searchBtn.focus();
			});

			$(document).off('keydown.searchToggleCommon').on('keydown.searchToggleCommon', function(e) {
				if(e.keyCode === 27 && $searchContainer.hasClass('active')) {
					$searchContainer.removeClass('active');
					$searchBtn.focus();
				}
			});
